...
/* 3D tilt helpers */
function calcRotate(e: React.MouseEvent, el: HTMLElement) {
  const rect = el.getBoundingClientRect();
  const x = e.clientX - rect.left;
  const y = e.clientY - rect.top;
  return {
    rotateX: ((y - rect.height / 2) / rect.height) * -12,
    rotateY: ((x - rect.width / 2) / rect.width) * 12,
  };
}
...
export const ProjectCard: FC<ProjectCardProps> = ({
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    const r = calcRotate(e, cardRef.current);
    setTilt({ x: r.rotateY, y: r.rotateX });
  }, []);
  ...
  return (
    ...
    >
      <Card
        ...
        className="relative overflow-hidden backdrop-blur-xl border transition-all duration-200 h-full flex flex-col shadow-xl hover:shadow-2xl group-hover:shadow-luxury-hover-glow/30 rounded-2xl"
        style={{
